...
async def async_setup_entry(
    ...
    for description in SENSOR_TYPES.values():
        if description.key in FAST_POLL_SENSORS:
            ...
            entities.append(
                SajSensor(hub, device_info, description, is_fast_variant=False)
            )
            entities.append(
                FastPollSensor(hub, device_info, description, is_fast_variant=True)
            )
        else:
    ...
    _LOGGER.info(
        "Added SAJ sensors (%d normal, %d fast-variants)", normal_count, fast_count
    )
...
class SajSensor(CoordinatorEntity, SensorEntity):
    ...
    def __init__(
        ...
        self._is_fast_sensor = (
            description.key in FAST_POLL_SENSORS
        ) and is_fast_variant
        ...
        if ADVANCED_LOGGING and self._is_fast_sensor:
            _LOGGER.debug(
                "Sensor %s (key: %s) marked as fast-poll sensor",
                self._attr_name,
                description.key,
            )
