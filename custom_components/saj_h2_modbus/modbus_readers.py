...
def _log_partial_errors(
    ...
    log_level = (
        logging.WARNING if log_level_on_error > logging.WARNING else log_level_on_error
    )
    _LOGGER.log(
        log_level,
...
ADDITIONAL_DATA_1_PART_2_MAP = [
    ("directionPV", None),
    ("directionBattery", "16i"),
    ("directionGrid", "16i"),
    ("directionOutput", None),
    (None, "skip_bytes", 14),
    ("TotalLoadPower", "16i"),
    ("CT_GridPowerWatt", "16i"),
    ("CT_GridPowerVA", "16i"),
    ("CT_PVPowerWatt", "16i"),
    ("CT_PVPowerVA", "16i"),
    ("pvPower", "16i"),
    ("batteryPower", "16i"),
    ("totalgridPower", "16i"),
    ("totalgridPowerVA", "16i"),
    ("inverterPower", "16i"),
    ("TotalInvPowerVA", "16i"),
    ("BackupTotalLoadPowerWatt", None),
    ("BackupTotalLoadPowerVA", None),
    ("gridPower", "16i"),
]
...
ADDITIONAL_DATA_3_MAP = [
    ("today_pv_energy2", "32u", 0.01),
    ("month_pv_energy2", "32u", 0.01),
    ("year_pv_energy2", "32u", 0.01),
    ("total_pv_energy2", "32u", 0.01),
    ("today_pv_energy3", "32u", 0.01),
    ("month_pv_energy3", "32u", 0.01),
    ("year_pv_energy3", "32u", 0.01),
    ("total_pv_energy3", "32u", 0.01),
    ("sell_today_energy_2", "32u", 0.01),
    ("sell_month_energy_2", "32u", 0.01),
    ("sell_year_energy_2", "32u", 0.01),
    ("sell_total_energy_2", "32u", 0.01),
    ("sell_today_energy_3", "32u", 0.01),
    ("sell_month_energy_3", "32u", 0.01),
    ("sell_year_energy_3", "32u", 0.01),
]

ADDITIONAL_DATA_3_2_MAP = [
    ("sell_total_energy_3", "32u", 0.01),
    ("feedin_today_energy_2", "32u", 0.01),
    ("feedin_month_energy_2", "32u", 0.01),
    ("feedin_year_energy_2", "32u", 0.01),
    ("feedin_total_energy_2", "32u", 0.01),
    ("feedin_today_energy_3", "32u", 0.01),
    ("feedin_month_energy_3", "32u", 0.01),
    ("feedin_year_energy_3", "32u", 0.01),
    ("feedin_total_energy_3", "32u", 0.01),
    ("sum_feed_in_today", "32u", 0.01),
    ("sum_feed_in_month", "32u", 0.01),
    ("sum_feed_in_year", "32u", 0.01),
    ("sum_feed_in_total", "32u", 0.01),
    ("sum_sell_today", "32u", 0.01),
    ("sum_sell_month", "32u", 0.01),
    ("sum_sell_year", "32u", 0.01),
...
BATTERY_DATA_MAP = [
    ...
    ("Bat1SOC", None),
    ("Bat1SOH", None),
    ("Bat1Voltage", None, 0.1),
    ("Bat1Current", "16i"),
    ("Bat1Temperature", "16i", 0.1),
    ("Bat1CycleNum", None, 1),
    ("Bat2SOC", None),
    ("Bat2SOH", None),
    ("Bat2Voltage", None, 0.1),
    ("Bat2Current", "16i"),
    ("Bat2Temperature", "16i", 0.1),
    ("Bat2CycleNum", None, 1),
    ("Bat3SOC", None),
    ("Bat3SOH", None),
    ("Bat3Voltage", None, 0.1),
    ("Bat3Current", "16i"),
    ("Bat3Temperature", "16i", 0.1),
    ("Bat3CycleNum", None, 1),
    ("Bat4SOC", None),
    ("Bat4SOH", None),
    ("Bat4Voltage", None, 0.1),
    ("Bat4Current", "16i"),
    ("Bat4Temperature", "16i", 0.1),
    ("Bat4CycleNum", None, 1),
    (None, "skip_bytes", 12),
    ("Bat1DischarCap", "32u", 1),
    ("Bat2DischarCap", "32u", 1),
    ("Bat3DischarCap", "32u", 1),
    ("Bat4DischarCap", "32u", 1),
    ("BatProtHigh", None, 0.1),
    ("BatProtLow", None, 0.1),
    ("Bat_Chargevoltage", None, 0.1),
    ("Bat_DisCutOffVolt", None, 0.1),
    ("BatDisCurrLimit", None, 0.1),
    ("BatChaCurrLimit", None, 0.1),
]
...
for i in range(7):
    ...
    CHARGE_DATA_MAP += [
        (f"charge{p}_start_time", "16u", 1),
        (f"charge{p}_end_time", "16u", 1),
...
PASSIVE_BATTERY_DATA_MAP = [
    ...
    (
        ...
        "16u",
        1,
    ),  # 0x3660: 0=Not allow, 1=Allow charge/discharge in time-sharing mode
]
...
async def _read_modbus_data(
    ...
    try:
        ...
        _LOGGER.log(
            log_level_on_error, "Error reading modbus data for %s: %s", data_key, e
        )
    ...
    if not regs:
        ...
        _LOGGER.log(
            log_level_on_error,
            "Error reading modbus data: No response for %s",
            data_key,
        )
        ...
        try:
            ...
            if method == "16i":
                value = client.convert_from_registers(
                    [raw_value], ModbusClientMixin.DATATYPE.INT16
                )
            elif method == "16u":
                value = client.convert_from_registers(
                    [raw_value], ModbusClientMixin.DATATYPE.UINT16
                )
            elif method == "32u":
                if index + 1 >= len(regs):
...
async def _read_configured_data(
    client: ModbusTcpClient, lock: Lock, config_key: str
) -> DataDict:
    """
    ...
    _log_partial_errors(
        config["data_key"], errors, config.get("log_level_on_error", logging.ERROR)
    )
    return data


async def _read_configured_phase_data(
    client: ModbusTcpClient, lock: Lock, config_key: str
) -> DataDict:
    """
...
async def read_modbus_realtime_data(client: ModbusTcpClient, lock: Lock) -> DataDict:
    ...
    for key in ["faultMsg0", "faultMsg1", "faultMsg2"]:
        ...
        fault_messages.extend(
            [
                msg
                for code, msg in FAULT_MESSAGES[int(key[-1])].items()
...
async def read_additional_modbus_data_4(
    client: ModbusTcpClient, lock: Lock
) -> DataDict:
...
def _decode_time_power_slots(data: DataDict, prefix: str, slots: int = 7) -> None:
    ...
    for i in range(slots):
        p = "" if i == 0 else str(i + 1)
        k_start, k_end, k_raw = (
            f"{prefix}{p}_start_time",
            f"{prefix}{p}_end_time",
            f"{prefix}{p}_power_raw",
        )
        if k_start in data:
    ...
    try:
        ...
        if data:
            ...
            if mode is not None:
                ...
                data["AntiRefluxCurrentmode"] = modes.get(
